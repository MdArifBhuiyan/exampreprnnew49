...
// Service fees
const serviceFees = {
  bkash: 0.02, // 2% service fee for bKash
  rocket: 0.015, // 1.5% service fee for Rocket
};
...
app.post('/bkash/create-payment', async (req, res) => {
  try {
    ...
    const baseAmount = amount / (1 + serviceFees.bkash); // Calculate base amount before fee
    const fee = amount - baseAmount;

    console.log(`bKash Payment: Base Amount: ${baseAmount}, Service Fee: ${fee}, Total: ${amount}`);
...
app.post('/rocket/create-payment', async (req, res) => {
  try {
    const { amount, userId } = req.body;
    const baseAmount = amount / (1 + serviceFees.rocket); // Calculate base amount before fee
    const fee = amount - baseAmount;

    console.log(`Rocket Payment: Base Amount: ${baseAmount}, Service Fee: ${fee}, Total: ${amount}`);
